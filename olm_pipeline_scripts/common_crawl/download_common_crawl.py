...
for index in range(len(args.common_crawl_snapshots)):
    ...
    run(f"mkdir ungoliant_downloads-{str(index)}", shell=True)
    run("wget https://data.commoncrawl.org/crawl-data/$new_crawl/wet.paths.gz", shell=True)
    ...
    run(f"ungoliant download -t={args.num_proc} -o={args.common_crawl_offsets[index]} {wet_paths_name} {download_dir_name}", shell=True)
    ...
    run(f"cd {download_dir_name}", shell=True)
    run(f"for f in * ; do mv \"$f\" {'0'*index}\"$f\" ; done", shell=True)
    run("cd ../", shell=True)
