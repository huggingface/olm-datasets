...
python data-preparation/preprocessing/training/01b_oscar_cleaning_and_filtering/main_filtering.py --dataset_name=$huggingface_username/ungoliant-for-olm-raw-$crawl-offset-$offset --config_name=Tristan--ungoliant-for-olm-raw-$crawl --lang_dataset_id=en --path_fasttext_model=sp_kenlm_ft_models/lid.176.bin --path_sentencepiece_model=sp_kenlm_ft_models/en.sp.model --path_kenlm_model=sp_kenlm_ft_models/en.arpa.bin --path_dir_save_dataset=data/oscar_filtered_final
python data-preparation/preprocessing/training/01b_oscar_cleaning_and_filtering/deduplicate/self_deduplicate.py data-preparation/preprocessing/training/01b_oscar_cleaning_and_filtering/deduplicate/conf/self_deduplicate_en.yaml
rm -r data
sed  -i '1i text' outputs/en/text.csv
python push_ungoliant_clean_to_hub.py $crawl-offset-$offset $num_proc $huggingface_username
